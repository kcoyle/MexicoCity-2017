...
Early libraries were valued for their holdings in a time when printed materials were rare and quite expensive. While libraries do hold some rare and precious materials, and their collections far exceed what one could amass as a personal library, the fact of today is that we have an abundance of documents and other information sources. It is no longer reasonable for anyone to approach a research collection without some selecting and filtering mechanism. Too often, searches in library catalogs produce a retrieved set that is much too large for human processing. Although FRBR includes "identify" and "select" these tasks cannot be done on retrieved sets of thousands. Facets allow some filtering, but we need to think about designing for facting, for clustering, and for data mining so that we can help users overcome the abundance problem.

### Inventory, or information system?

There is no question that every library needs an inventory of what it owns and what it licenses. This serves planning, collection development, and is often a fiscal requirement. We cannot expect that the inventory, which is identifies individual items, will also serve the needs of researchers and other library users. When library catalogs were mere author/title lists, users were expected to consult bibliographies in order determine what book to ask for at the library desk. With a library catalog that is richer and more accessible, the use of bibliographies has diminished, but the catalog does not actually replace the bibliography function. Most bibliographies are selective; they are organized by subject matter or other themes; they can contain abstracts describing the reason the item was included. In essence, bibliographies are a selection tool. The FRBR "select" cannot be done on a mere listing of library holdings, but requires some organization and evaluation of the resources retrieved. The larger the library, the more support is needed to achieve selection, but our catalogs today do not include the necessary information to make this possible.

### The library in the world

There was a time when the library was the main information world available; today it is a mere speck within a much larger world of information. Users live and work in that larger world and spend much more time there than they do in the library. Keeping the library catalog closed to that larger world means isolating catalog users from the wealth of information that exists elsewhere. On the Web a user can find detailed information about a topic, an author, or a work. While they are using the library catalog that information is not available to them. The information landscape is increasingly open, interactive, and fast-moving. The walled garden of the library catalog looks limited and old-fashioned to today's users. We need to break down the walls and bring a richer service to our users than a mere listing of bibliographic details.

## The Conclusion

I would very much like to present answers to the many questions that I raise here, but this is something that we must do together. I hope we can initiate an exchange of ideas with open minds and hearts.
